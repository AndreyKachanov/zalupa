    ...
    <div class="row justify-content-center">
        ...
        >
            <div class="card h-100">
                <div class="budgets" :class="singleItemClass(item)" v-if="item.is_new || item.is_hit || item.is_bestseller">
                    ...
                    <div class="container">
                        <div class="row justify-content-between align-items-center">
                            <div class="col-6 pl-0 pr-0">
                                <count-items :count-from-cart="getCountFromCart(item.id)" @set-cnt="setNewCnt(item.id, $event)"></count-items>
                            </div>
                            <div class="col-5 cart-buttons pl-0 pr-0">
                                <button style="width: 100%;" v-if="inCart(item.id)" class="btn btn-sm btn-danger" @click="removeFromCart(item.id)">
                                    Убрать
                                </button>
                                <button style="width: 100%;" v-else class="btn btn-sm btn-success" @click="addToCartNew(item.id)">
                                    В корзину
                                </button>
                            </div>
                        </div>
